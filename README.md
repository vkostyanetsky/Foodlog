...
6. Текущее потребление калорий (сколько можно съесть и сколько съедено) можно автоматически отправлять в Телеграм при каждом изменении дневника питания (см. параметры ). Для этого заполните параметры интеграции (`telegram_bot_api_token`, `telegram_chat_id`) и настройте регулярный запуск скрипта [send_statistics.py](scripts/send_statistics.py).
